...
export default function SpentSection({ items, onAdd, onDelete, onExportCSV }: SpentSectionProps) {
  ...
  const handleDeleteAttempt = async (id: string) => {
    if (deletePassword === 'streetboys2026') {
      await onDelete(id);
                              ...
                              {confirmDelete === exp.id ? (
                                  <div className="flex items-center gap-2 justify-center">
                                    <input
                                        ...
                                        className="w-16 bg-white/5 border border-white/10 rounded px-1.5 py-1 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-rose-500/50"
                                        autoFocus
                                    />
                                    <button onClick={() => handleDeleteAttempt(exp.id)} className="text-rose-400 hover:text-rose-300 text-xs font-medium">
                                      Del
                                    </button>
                                    <span className="text-slate-600">/</span>
                                    <button onClick={cancelDelete} className="text-slate-400 hover:text-slate-300 text-xs">
                                      X
                                    </button>
                                  </div>
                              ) : (
